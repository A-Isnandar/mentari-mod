...
(function () {
  ...
  // Fungsi untuk kirim prompt ke Gemini dengan Retry Otomatis (Anti-429)
  async function askGeminiWithContent(
    ...
    // -- BAGIAN PROMPT TETAP SAMA KAYAK SEBELUMNYA --
    if (mode === 'default') {
      prompt = `Bacalah diskusi berikut, lalu tuliskan jawaban atau rangkuman yang natural... (teks prompt asli lu) ... \n${content}\n\nJawaban yang natural dan mengalir:`;
    } else if (mode === 'shorten') {
      prompt = `Ringkas jawaban berikut menjadi lebih singkat... \n\n${prevAnswer}`;
    } else if (mode === 'paragraph') {
      prompt = `Ubah jawaban berikut menjadi satu paragraf... \n\n${prevAnswer}`;
    } else if (mode === 'clarify') {
      prompt = `Perjelas dan buat jawaban berikut lebih mudah dipahami... \n\n${prevAnswer}`;
    } else if (mode === 'lengthen') {
      prompt = `Perpanjang dan tambahkan detail... \n\n${prevAnswer}`;
    } else if (mode === 'numbered') {
      prompt = `Ubah jawaban berikut menjadi nomor... \n\n${prevAnswer}`;
    }

    // -- INI BAGIAN PENTING YANG DIUBAH --
    // ganti ke gemini-2.5-flash-lite biar lebih stabil kuotanya
    ...
    try {
      ...
      // Kalo kena limit (429), kita coba lagi (maksimal 3x retry)
      if (response.status === 429 && retryCount < 3) {
        const waitTime = Math.pow(2, retryCount) * 1000 + Math.random() * 500; // Nunggu: 1s, 2s, 4s...
        console.warn(
          `Kena limit 429, nyoba lagi dalam ${Math.round(waitTime)}ms...`
        );
        await new Promise((r) => setTimeout(r, waitTime));
        return askGeminiWithContent(content, mode, prevAnswer, retryCount + 1);
      }
      ...
      // Kalo errornya bukan 429, langsung lempar aja biar user tau
      throw new Error(
        'Gagal mendapatkan jawaban dari Gemini: ' + error.message
      );
    }
  }
